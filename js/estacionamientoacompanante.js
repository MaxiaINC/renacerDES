...
$("#requiere_acompanante").on('select2:select', function(e) {
	...
	let opcionSeleccionada = e.params.data.id;
...
let limpiarFormAc = () => {
	$("#form_solicitud_acompanante")[0].reset();
	$("#form_solicitud_acompanante").find('select').each(function() {
...
corregimientos_ac(0);
	...
	function guardar_acompanante(){
		...
		if(validarformNA(datos) == 1){
			$.ajax({
				type: 'post',
				url: 'controller/acompanantesestacionamientosback.php',
				dataType: 'json',
				data: { 
