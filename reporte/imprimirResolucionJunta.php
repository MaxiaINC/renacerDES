<?php
	...
	class PDF extends FPDF{
    	...
    	function Header(){
			...
			if($numPag > 1){
				$this->Cell(0,3,'P�g. '.$this->PageNo(),0,1,'R');	
				$this->Cell(0,3,'Resoluci�n N� '.$nroresolucion,0,0,'R');	
	...
	$pdf->Cell(0,5,'RESOLUCI�N N� '.$nroresolucion,'0',1,'C');   
   ...
   //$pdf->Ln(5);
	...
	$pdf->Ln(3);
	$pdf->SetFont('Arial','',12);  
	$pdf->Cell(0,5,'Que mediante Ley N� 23 de 28 de junio de 2007, se cre� la Secretar�a Nacional de',0,1,'FJ');	
	$pdf->Cell(0,5,'Discapacidad (SENADIS), como entidad aut�noma del Estado, con el fin de dirigir',0,1,'FJ'); 
	$pdf->Cell(0,5,'y ejecutar la pol�tica de inclusi�n social de las personas con discapacidad y sus',0,1,'FJ'); 
	$pdf->Cell(0,5,'familias;',0,1,'L'); 
	$pdf->Ln(3); 
	$pdf->Cell(0,5,'Que los numerales 9 y 10 del Art�culo 13 de la Ley supra citada, establecen como',0,1,'FJ'); 
	$pdf->Cell(0,5,'funciones de la Secretar�a Nacional de Discapacidad, el dise�o de los baremos',0,1,'FJ'); 
	$pdf->Cell(0,5,'nacionales necesarios para valorar la discapacidad que sustentan la emisi�n de la',0,1,'FJ'); 
	$pdf->Cell(0,5,'certificaci�n de discapacidad;',0,1,'L'); 
	$pdf->Ln(3);
	$pdf->Cell(0,5,'Que mediante Decreto Ejecutivo N� 37 de 11 de abril de 2014, modificado',0,1,'FJ'); 
	$pdf->Cell(0,5,'mediante Decreto Ejecutivo N� 18 de 24 de febrero de 2015, se cre� la Direcci�n',0,1,'FJ'); 
	$pdf->Cell(0,5,'Nacional de Certificaciones, la cual conforme al art�culo 2, numeral 4, del  texto',0,1,'FJ'); 
	$pdf->Cell(0,5,'legal citado, tiene como objetivos supervisar, dirigir y controlar los procesos de',0,1,'FJ'); 
	$pdf->Cell(0,5,'evaluaci�n, valoraci�n y emisi�n de la certificaci�n de la discapacidad;',0,1,'L'); 
	$pdf->Ln(3);
	$pdf->Cell(0,5,'Que mediante Decreto Ejecutivo N� 36 de 11 de abril de 2014, modificado',0,1,'FJ');
	$pdf->Cell(0,5,'mediante Decreto Ejecutivo N� 74 de 14 de abril de 2015, se reglament� el',0,1,'FJ');
	$pdf->Cell(0,5,'procedimiento de conformaci�n y funcionamiento de las Juntas Evaluadoras de la',0,1,'FJ');
	$pdf->Cell(0,5,'Discapacidad, los baremos nacionales y se dict� el procedimiento para la',0,1,'FJ');
	$pdf->Cell(0,5,'evaluaci�n, valoraci�n y certificaci�n de la discapacidad;',0,1,'L');
	$pdf->Ln(3);
	$pdf->Cell(0,5,'Que SENADIS atendiendo al tenor del art�culo 7 del Decreto Ejecutivo N� 36 de 11 ',0,1,'FJ');
	$pdf->Cell(0,5,'de abril de 2014, tiene la responsabilidad de conformar las Juntas Evaluadoras de ',0,1,'FJ');
	$pdf->Cell(0,5,'la Discapacidad, las cuales tienen como funci�n ejecutar el procedimiento de ',0,1,'FJ');
	$pdf->Cell(0,5,'evaluaci�n, valoraci�n y certificaci�n de las personas que de manera voluntaria ',0,1,'FJ');
	$pdf->Cell(0,5,'soliciten la certificaci�n de discapacidad;',0,1,'L');
	$pdf->Ln(3);
	$pdf->Cell(0,5,'Que conforme a lo establecido en los art�culos 4 y 5, del Decreto Ejecutivo N� 74 ',0,1,'FJ');
	$pdf->Cell(0,5,'de 14 de abril de 2015, cada Junta Evaluadora de la Discapacidad deber� estar',0,1,'FJ');
	$pdf->Cell(0,5,'integrada por un m�nimo tres (3) miembros y se deber�n conformar ',0,1,'FJ');
	$pdf->Cell(0,5,'interdisciplinariamente, siendo requisito que sus miembros posean t�tulo',0,1,'FJ');
	$pdf->Cell(0,5,'universitario en Medicina, Psicolog�a, Fisioterapia, Fonoaudiolog�a, Terapia',0,1,'FJ');
	...
	$pdf->AddPage();
	$pdf->setY(38);
	$pdf->Cell(0,5,'Que el art�culo 6, del Decreto Ejecutivo N� 74 de 14 de abril de 2015, antes citado,',0,1,'FJ');
	...
	$pdf->Cell(0,5,'Evaluar a todas las personas que soliciten la certificaci�n de discapacidad. Para',0,1,'FJ');
	$pdf->setX(40);
	$pdf->Cell(0,5,'ello las Juntas Evaluadoras de la Discapacidad, en el marco del Art�culo 5, sobre ',0,1,'FJ');
	$pdf->setX(40);
	$pdf->Cell(0,5,'Igualdad y no discriminaci�n de la Convenci�n sobre los Derechos de las ',0,1,'FJ');
	$pdf->setX(40);
	$pdf->Cell(0,5,'Personas con Discapacidad, efectuar� giras itinerantes en el interior del pa�s. ',0,1,'FJ');
	$pdf->Ln(3);
	$pdf->setX(32);
	$pdf->Cell(8,5,'2.	',0,0,'L');
	$pdf->Cell(0,5,'Completar el protocolo de evaluaci�n y los formularios a partir de los cuales se',0,1,'FJ');
	$pdf->setX(40);
	$pdf->Cell(0,5,'obtendr� la informaci�n para valorar a la persona, siguiendo los procedimientos',0,1,'FJ');
	$pdf->setX(40);
	$pdf->Cell(0,5,'establecidos en los manuales que se aprueben para ello.',0,1,'L');
	$pdf->Ln(3);
	$pdf->setX(32);
	$pdf->Cell(8,5,'3.	',0,0,'L');
	$pdf->Cell(0,5,'Recopilar informaci�n cl�nica, social, laboral y escolar de las personas que soliciten ',0,1,'FJ');
	$pdf->setX(40);
	$pdf->Cell(0,5,'la evaluaci�n para la certificaci�n de discapacidad. ',0,1,'L');
	$pdf->Ln(3);
	$pdf->setX(32);
	...
	$pdf->setX(40);
	$pdf->Cell(0,5,'Codificar las categor�as que describen el perfil de funcionamiento de la persona, ',0,1,'FJ');
	$pdf->setX(40);
	$pdf->Cell(0,5,'seg�n la CIF, acorde a lo requerido en cada caso.  ',0,1,'L');
	$pdf->Ln(3);
	$pdf->setX(32);
	$pdf->Cell(8,5,'6.	',0,0,'L');
	$pdf->setX(40);
	$pdf->Cell(0,5,'Firmar y sellar el protocolo de evaluaci�n.',0,1,'L');
	$pdf->Ln(3);
	$pdf->setX(32);
	$pdf->Cell(8,5,'7.	',0,0,'L');
	$pdf->setX(40);
	$pdf->Cell(0,5,'Firmar y sellar el formulario que sustenta el otorgamiento de la certificaci�n de ',0,1,'FJ');
	$pdf->setX(40);
	...
	$pdf->setX(40);
	$pdf->Cell(0,5,'Firmar y sellar el formulario de denegatoria de la certificaci�n de discapacidad, ',0,1,'FJ');
	$pdf->setX(40);
	...
	$pdf->setX(40);
	$pdf->Cell(0,5,'Brindar orientaci�n y asesor�a en lo que conlleva el proceso de certificaci�n de la ',0,1,'FJ');
	$pdf->setX(40);
	$pdf->Cell(0,5,'discapacidad, a las personas que as� lo soliciten',0,1,'L');
	$pdf->Ln(3);
	$pdf->setX(32);
	$pdf->Cell(8,5,'10. ',0,0,'L');
	$pdf->setX(40);
	$pdf->Cell(0,5,'Mantener la confidencialidad y privacidad de la informaci�n contenida en los ',0,1,'FJ');
	$pdf->setX(40);
	$pdf->Cell(0,5,'expedientes de las personas que soliciten la certificaci�n de discapacidad acorde a ',0,1,'FJ');
	$pdf->setX(40);
	$pdf->Cell(0,5,'lo que establece la Ley N� 68 de 20 de noviembre de 2003, que regula los ',0,1,'FJ');
	$pdf->setX(40);
	$pdf->Cell(0,5,'derechos y obligaciones de los pacientes, en materia de informaci�n y de decisi�n ',0,1,'FJ');
	$pdf->setX(40);
	$pdf->Cell(0,5,'libre e informada.',0,1,'L');
	$pdf->Ln(3);
	$pdf->SetFont('Arial','',12); 
	$pdf->Cell(0,5,'Que del mismo modo, el art�culo 7, del Decreto Ejecutivo N� 74 de 14 de abril de ',0,1,'FJ');
	$pdf->Cell(0,5,'2015, establece que los miembros de las Juntas Evaluadoras de la Discapacidad ',0,1,'FJ');
	$pdf->Cell(0,5,'ser�n escogidos y designados por el o la Directora de la Secretar�a Nacional de ',0,1,'FJ');
	$pdf->Cell(0,5,'Discapacidad, previo el cumplimiento del procedimiento de postulaci�n y selecci�n  ',0,1,'FJ');
	$pdf->Cell(0,5,'contemplado en el art�culo 14 del Decreto Ejecutivo N� 36 de 11 de abril de 2014; ',0,1,'FJ');
	$pdf->Cell(0,5,'modificado por el art�culo 8 del Decreto Ejecutivo N� 74 de 14 de abril de 2015;',0,1,'L');
	$pdf->Ln(6);
	$pdf->Cell(0,5,'Que una vez cumplido, el procedimiento de postulaci�n, selecci�n y ',0,1,'FJ');
	$pdf->Cell(0,5,'nombramiento, arriba descrito, la Directora Nacional de la Secretar�a de ',0,1,'FJ');
	$pdf->Cell(0,5,'Discapacidad, mediante Resoluci�n N� 71 de 21 de septiembre de 2021, nombra a ',0,1,'FJ');
	$pdf->Cell(0,5,'la  Directora Nacional de Certificaciones quien queda facultada para instalar la ',0,1,'FJ');
	$pdf->Cell(0,5,'Junta Evaluadora, para cada proceso de evaluaci�n, valoraci�n y certificaci�n de ',0,1,'FJ');
	$pdf->Cell(0,5,'la discapacidad;',0,1,'L');
	$pdf->Ln(6);
	$pdf->Cell(0,5,'Que en virtud de lo dispuesto en la Resoluci�n antes mencionada, el Director ',0,1,'FJ');
	$pdf->Cell(0,5,'Nacional de Certificaciones de la Secretar�a Nacional de Discapacidad;',0,1,'FJ');
	$pdf->Ln(5);
	$pdf->SetFont('Arial','B',12);  
	...
	$pdf->Cell(30, 10, 'C�dula', 'LTRB', 0, 'C');
	$pdf->Cell(50, 10, 'Profesi�n', 'LTRB', 1, 'C');
	...
	$pdf->Cell(0,5,'mediante la presente resoluci�n, quedan facultadas para evaluar conforme el ',0,1,'FJ');
	$pdf->Cell(0,5,'procedimiento contemplado en el Decreto Ejecutivo N� 36 de 11 de abril de 2014, ',0,1,'FJ');
	$pdf->Cell(0,5,'a las personas descritas a continuaci�n:',0,1,'L');
	
	$pdf->Ln(5);
	
	$pdf->SetFont('Arial','B',12);
	$pdf->Cell(121,10,'Nombre',1,0,'C');
	$pdf->Cell(40,10,'C�dula',1,1,'C');
	...
	$pdf->SetLineWidth(0);
	$pdf->SetFont('Arial','',12); 
	$pdf->Cell(0,5,'La presente resoluci�n entrar� a regir a partir de la fecha de su firma',0,1,'FJ');
	$pdf->Ln(3);
	$pdf->SetFont('Arial','B',12);
	$pdf->Cell(65,5,'FUNDAMENTO DE DERECHO.',0,0,'FJ');
	$pdf->SetFont('Arial','',12);
	$pdf->Cell(0,5,' Art�culos 1 y 13 de la Ley N� 23 de 28 de junio de ',0,1,'FJ');
	$pdf->Cell(0,5,'2007; Cap�tulo II del Decreto Ejecutivo N� 36 de 11 de abril de 2014;  Art�culos 1  y ',0,1,'FJ');
	$pdf->Cell(0,5,'2  del Decreto Ejecutivo N� 18 de 24 de febrero de 2015, que modifica el  Decreto ',0,1,'FJ');
	$pdf->Cell(0,5,'Ejecutivo N� 37 de 11 de abril de 2014, y Art�culos 4, 5, 6, 7 y 8 del Decreto ',0,1,'FJ');
	...
	$pdf->Ln(3); 
	$pdf->Cell(0,5,'Dada en la Ciudad de Panam�, el d�a '.letras($dia_resolucion).' ('.$dia_resolucion.') del mes de '.$mes_nom_resolucion.' del dos mil ',0,1,'FJ');
	...
	$pdf->Ln(10);
	$pdf->SetFont('Arial','B',12);
	$pdf->Cell(0,5,'P�BLIQUESE Y C�MPLASE',0,1,'C');
	$y1 = $pdf->getY(); 
