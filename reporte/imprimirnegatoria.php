<?php
	...
	$pdf->Cell(0,6,utf8_decode('Resolución N°  '.$resolucion),'0',1,'C'); 
	$pdf->Ln(5);
	$pdf->Cell(0,6,utf8_decode('Por medio de la cual se NIEGA la Certificación de Discapacidad a '.utf8_decode(trim($nombre).' '.trim($apellidopaterno).' '.trim($apellidomaterno)).''),'0',1,'C'); 
	$pdf->Ln(5);
	$pdf->Cell(0,6,utf8_decode('LA DIRECTORA NACIONAL DE CERTIFICACIONES DE LA'),'0',1,'C'); 
	$pdf->Cell(0,6,utf8_decode('SECRETARÍA NACIONAL DE DISCAPACIDAD'),'0',1,'C');
	$pdf->Ln(5);
	...
	$pdf->Ln(5);
	$pdf->SetFont('Arial','',11);
	$pdf->MultiCell(0,6,utf8_decode('Que mediante Ley N.° 23 de 28 de junio de 2007, se creó la Secretaría Nacional de Discapacidad (SENADIS), como entidad autónoma del Estado, con el fin de dirigir y ejecutar la política de inclusión social de las personas con discapacidad y sus familias;'),0,'J',1);
	$pdf->Ln(4);
	$pdf->MultiCell(0,6,utf8_decode('Que los numerales 9 y 10 del Artículo 13 de la Ley supra citada, establecen como funciones de la Secretaría Nacional de Discapacidad, el diseño de los baremos nacionales necesarios para valorar la discapacidad que sustentan la emisión de la certificación de discapacidad;'),0,'J',1);
	$pdf->Ln(4);
	...
	$pdf->MultiCell(0,6,utf8_decode('Que el día '.$fechasol_neg.', acudió a la Dirección Nacional de Certificaciones, '.$nombree.', con '.$tipo_documentoe.' '.$cedula.', con domicilio ubicado en '.$direccione.', quien de manera voluntaria, solicitó a dicha Dirección la correspondiente evaluación, valoración y certificación de su discapacidad,'),0,'J',1);

	$pdf->Ln(4);
	$pdf->MultiCell(0,6,utf8_decode('Que el día '.$fechae_neg.', tal cual lo exigido por el artículo 74, del Decreto N.° 36 de 2014, antes descrito, la Dirección Nacional de Certificaciones, le otorgó un cupo a '.$nombree.', para que la Junta Evaluadora, efectuase la correspondiente evaluación de su condición de salud;'),0,'J',1);
	$pdf->Ln(4);
	$pdf->MultiCell(0,6,utf8_decode('Que conforme a lo establecido en el artículo 4, del Decreto Ejecutivo N.° 74 de 14 de abril de 2015, la Junta Evaluadora de la Discapacidad integrada por un mínimo tres (3) miembros, y cumpliendo con el requisito de interdisciplinariedad, según consta en la Resolución de conformación de Junta Evaluadora N.° '.$codigojunta.' de la Dirección Nacional de Certificaciones, evaluó y valoró a la solicitante;'),0,'J',1);
	$pdf->Ln(4);
	$pdf->MultiCell(0,6,utf8_decode('Que en virtud de ello, la Junta Evaluadora, luego de evaluar y valorar de conformidad a los protocolos establecidos, en el Decreto Ejecutivo N.° 36 de 11 de abril de 2014, modificado mediante Decreto Ejecutivo N° 74 de 14 de abril de 2015, ha determinado que, '.$nombree.' con '.$tipo_documentoe.' '.$cedula.' no cumple con ninguno de los criterios que rigen para certificar como persona con Discapacidad '.utf8_decode($discapacidad).' '.$articulo_conclusion.'., según el informe remitido por dicha Junta, la que sustenta su decisión en función de los siguientes aspectos:'),0,'J',1);
	$pdf->Ln(4);
	$pdf->SetFont('Arial','B',11);
	$pdf->MultiCell(0,6,utf8_decode('Esta decisión se sustenta:'),'0',1,'C');
	$pdf->Ln(4);
	$pdf->SetFont('Arial','',11);
	$pdf->MultiCell(0,6,utf8_decode('1.	En la evaluación diagnóstica que presentó '.$nombree.' con '.$tipo_documentoe.' '.$cedula.' emitida por su médico tratante el cual refiere que presenta:'),0,'J',1);
	$pdf->Cell(10,6,'','0',0,'C');
	$pdf->MultiCell(0,6,utf8_decode($evaluacion),'0',1,'C');
	$pdf->Ln(4);
	$pdf->SetFont('Arial','B',11);
	$pdf->MultiCell(0,6,utf8_decode('        Diagnóstico según CIE10'),'0',1,'C');
	$pdf->SetFont('Arial','',11);
	...
	$pdf->Cell(58,5,utf8_decode('        Actividad y Participación: '),'',0,'L');  //76 
	$pdf->SetFont('Arial','',11);
	...
	$pdf->Ln(4);
	$pdf->SetFont('Arial','B',11);
	$pdf->MultiCell(0,5,utf8_decode('La Junta Evaluadora tomando en cuenta tanto las evidencias presentadas por la interesada, así como la evaluación y valoración realizada por la Junta Evaluadora de Discapacidad, aclara que de acuerdo al artículo antes mencionado:'),0,'J',1);
	...
	$pdf->MultiCell(0,5,utf8_decode('Que mediante Resolución No. 006-2023 de 19 de enero de 2023, se designó a '.$nombre_encargado.', como '.$cargo_encargado.' de la Secretaria Nacional de Discapacidad, siendo facultada para supervisar, dirigir y controlar los procesos de evaluación, valoración y emisión de la certificación de discapacidad, ademós de desempeñar las funciones técnicas y/o administrativas, relacionadas con las tareas adscritas a la Dirección.'),0,'J',1);
	$pdf->Ln(4);
	$pdf->MultiCell(0,5,utf8_decode('Que por todas las consideraciones expuestas, la Directora de la Dirección Nacional de Certificaciones de la Secretaría Nacional de Discapacidad;'),0,'J',1);
	$pdf->Ln(8);
	$pdf->SetFont('Arial','B',11);
	$pdf->Cell(0,5,utf8_decode('RESUELVE:'),'',1,'C');
	$pdf->Ln(4);
	$pdf->SetFont('Arial','',11);	
	$pdf->MultiCell(0,5,utf8_decode('PRIMERO: DENEGAR LA CERTIFICACIÓN DE LA DISCAPACIDAD a '.$nombree.', con '.$tipo_documentoe.' '.$cedula.' con expediente '.$expediente.', de la Dirección Nacional de Certificaciones de la Secretaría Nacional de Discapacidad.'),0,'J',1);
	$pdf->Ln(4);
	$pdf->MultiCell(0,5,utf8_decode('SEGUNDO: Notificar a la interesada el contenido de la presente Resolución y entregarle copia de la misma.'),0,'J',1);
	$pdf->Ln(4);
	$pdf->MultiCell(0,5,utf8_decode('TERCERO: Advertir que contra esta resolución puede interponerse el recurso de reconsideración, dentro del término de los cinco (5) días hábiles, contados a partir de su notificación.'),0,'J',1);
	$pdf->Ln(4);
	$pdf->MultiCell(0,5,utf8_decode('CUARTO: La presente resolución entrará a regir a partir de la fecha de su notificación.'),0,'J',1);
	$pdf->SetFont('Arial','B',11);
	$pdf->Ln(6);
	...
	$pdf->Ln(6);
	$pdf->SetFont('Arial','B',11);
	$pdf->Cell(0,5,utf8_decode('NOTIFÍQUESE Y CÚMPLASE '),'',1,'L');
	$valorY = $pdf->GetY();
	$pdf->Ln(50);
	$pdf->SetY($valorY+50);
